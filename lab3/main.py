"""
GUI Laboratorio 2 - Banco de filtros por FFT (Tkinter)

Funciones principales del lab:
- Entrenar: M grabaciones por cada uno de 2 comandos. Normaliza longitud a N y calcula energías por K subbandas.
...
- Visualizar: espectro (dB) y barras de energía por sub-banda.
...
N = 4096           # tamaño FFT por defecto
K = 4              # subbandas por defecto
M = 8              # grabaciones por defecto
WINDOW = "hamming" # ventana por defecto
MODEL_PATH = "lab2_model.json"
...
class Lab2GUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Lab 2 - Banco de filtros (FFT)")
        self.model = None
    ...
    def _build_ui(self):
        ...
        ttk.Combobox(params, values=["hamming","hann","rect"], textvariable=self.window_var, state='readonly', width=10).grid(row=0, column=9, padx=4)
        ...
        self.dev_cb = ttk.Combobox(entry, values=self.device_labels, textvariable=self.device_var, state='readonly', width=60)
        ...
        # Entrenamiento
        trainf = ttk.LabelFrame(main, text="Entrenamiento", padding=10)
        ...
        ttk.Button(trainf, text=f"Grabar M por etiqueta", command=self._record_M_per_label).grid(row=0, column=4, padx=6)
        ttk.Button(trainf, text="Entrenar desde carpetas", command=self._train_from_dirs).grid(row=0, column=5, padx=6)
        ttk.Button(trainf, text="Guardar modelo", command=self._save_model_dialog).grid(row=0, column=6, padx=6)
        ttk.Button(trainf, text="Cargar modelo", command=self._load_model_dialog).grid(row=0, column=7, padx=6)

        # Reconocimiento
        recog = ttk.LabelFrame(main, text="Reconocimiento", padding=10)
        recog.pack(fill=tk.X, pady=(8,0))
        ttk.Button(recog, text="Reconocer 1 toma (N/fs)", command=self._recognize_mic).grid(row=0, column=0, padx=6)
        ttk.Button(recog, text="Seleccionar WAV", command=self._pick_file).grid(row=0, column=1, padx=6)
        ttk.Label(recog, textvariable=self.file_path_var).grid(row=0, column=2, padx=6, sticky='w')
        ttk.Button(recog, text="Predecir archivo", command=self._predict_file).grid(row=0, column=3, padx=6)
        ttk.Button(recog, text="Iniciar RT (5s buffer)", command=self._start_rt).grid(row=1, column=0, padx=6, pady=(6,0))
        ttk.Button(recog, text="Detener RT", command=self._stop_rt).grid(row=1, column=1, padx=6, pady=(6,0))
        ttk.Label(recog, text="Pred:").grid(row=1, column=2, padx=6, pady=(6,0), sticky='e')
        ttk.Label(recog, textvariable=self.pred_label_var, foreground='#b91c1c', font=(None, 11, 'bold')).grid(row=1, column=3, padx=6, pady=(6,0), sticky='w')
    ...
    def _record_M_per_label(self):
        fs = int(self.fs_var.get()); N = int(self.N_var.get()); M = int(self.M_var.get())
        labels = [self.labA_var.get().strip(), self.labB_var.get().strip()]
    ...
    def _train_from_dirs(self):
        fs = int(self.fs_var.get()); N = int(self.N_var.get()); K = int(self.K_var.get()); M = int(self.M_var.get())
        base = self.dir_var.get().strip()
        mapping = {self.labA_var.get().strip(): self.labA_var.get().strip(),
    ...
    def _save_model_dialog(self):
        if self.model is None:
            ...
            return
        path = filedialog.asksaveasfilename(title='Guardar modelo JSON', defaultextension='.json', filetypes=[('JSON','*.json')])
        if not path:
            return
    ...
    def _load_model_dialog(self):
        path = filedialog.askopenfilename(title='Cargar modelo JSON', filetypes=[('JSON','*.json')])
        if not path:
            return
        self.model = load_model(path)
        self._log(f"Modelo cargado: {os.path.basename(path)}")

    def _recognize_mic(self):
        if self.model is None:
            messagebox.showwarning('Sin modelo', 'Cargue o entrene un modelo primero')
            return
        fs = self.model['fs']; N = self.model['N']; K = self.model['K']; window = self.model['window']
    ...
    def _predict_file(self):
        ...
        fs = self.model['fs']; N = self.model['N']; K = self.model['K']; window = self.model['window']
        if fsf != fs:
            messagebox.showerror('Fs incompatible', f'El WAV tiene fs={fsf} y el modelo espera fs={fs}. Remuestrea/normaliza antes.')
            return
        x = x[:N] if len(x) >= N else np.pad(x, (0, N - len(x)))
        Es, bands, freqs = compute_subband_energies(x, fs, N, K, window)
        label, dists = decide_label_by_min_dist(Es, self.model)
    ...
    def _update_plots(self, x: np.ndarray, fs: int, N: int, window: str, freqs: np.ndarray, Es: np.ndarray, label: str, mode: str | None = None):
        # Estilos y colores
        spec_color = "#2563eb"  # azul
        face = "#f9fafb"        # gris muy claro
        bar_palette = ["#10b981", "#3b82f6", "#f59e0b", "#ef4444", "#a855f7", "#14b8a6"]

        self.ax_spec.cla(); self.ax_bar.cla()
        self.ax_spec.set_facecolor(face); self.ax_bar.set_facecolor(face)
        self.ax_spec.grid(True, alpha=0.25)

        if mode == "silence":
            # Placeholder de silencio
            self.ax_spec.text(0.5, 0.5, 'Silencio', ha='center', va='center', color="#6b7280", transform=self.ax_spec.transAxes)
            self.ax_spec.set_title('Espectro |X(k)|')
            self.ax_spec.set_xlabel('Frecuencia (rad/s)'); self.ax_spec.set_ylabel('Magnitud |X(k)|')
            self.ax_bar.text(0.5, 0.5, 'Sin actividad', ha='center', va='center', color="#6b7280", transform=self.ax_bar.transAxes)
    ...
    def _start_input_meter(self):
        ...
        try:
            self.in_stream = sd.InputStream(samplerate=fs, channels=1, callback=cb, blocksize=block, device=dev)
    ...
    def _start_rt(self):
        ...
        if fs != int(self.model['fs']):
            messagebox.showerror('Fs incompatible', f'La GUI usa fs={fs} y el modelo espera fs={self.model["fs"]}. Ajusta FS.')
            return
        self.pred_label_var.set('-'); self.pred_dists_var.set('-')
        self.recognizer_stop.clear()
        self.prev_rms = 0.0; self.noise_rms = 0.0
    ...
    def _rt_worker(self):
        fs = int(self.fs_var.get()); N = int(self.N_var.get()); K = int(self.K_var.get()); window = self.window_var.get()
        ...
        while not self.recognizer_stop.is_set():
            ...
            if is_silence:
                ...
                if (now - self.last_activity_time) > self.silence_min_time:
                    ...
                    self._update_plots(buf, fs, N, window, np.linspace(0, fs/2, N//2+1), np.zeros(K), '', mode="silence")
                else:
